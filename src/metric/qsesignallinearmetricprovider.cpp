...
QseSignalLinearMetricProvider::QseSignalLinearMetricProvider(QObject *parent)
...
QseSignalLinearMetricProvider::~QseSignalLinearMetricProvider()
{
    delete m_mapper;
}

void QseSignalLinearMetricProvider::setFactor(double factor)
{
    if (m_factor != factor) {
        m_factor = factor;
        emit changed();
...
int QseSignalLinearMetricProvider::maximumTextLenght() const
{
    return 8;
}

QList<QseMetricItem> QseSignalLinearMetricProvider::create(const QseGeometry &geometry,
                                                           int size) const
{
